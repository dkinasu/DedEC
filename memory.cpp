...
struct fp_node *Init_fp_node(struct traceline *T_line)
{
	...
	if (fingerprint != NULL)
	{
		memcpy(new_node->fingerprint, fingerprint, strlen(fingerprint)+1);
...
struct fp_node * Find_fp(char * fp)
{
	...
	for(s = fp_store; s != NULL; s = (struct fp_node *) s->hh.next) 
    {
		...
		//printf("length_s->fingerprint: %lu  len_fp: %lu\n", strlen(s->fingerprint), strlen(fp));
    	k = strcmp(s->fingerprint, fp);
    	if(k == 0)
    	{
    		//printf("Equal!\n");
    		return s;
